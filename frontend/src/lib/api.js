...
export async function applyFix(diagnostic_id, branch_name, commit_message) {
  return fetchAPI('/api/fixes/apply', {
    method: 'POST',
    body: JSON.stringify({ diagnostic_id, branch_name, commit_message }),
  });
}

export async function getFixes() {
  return fetchAPI('/api/fixes');
}

export async function deleteFix(fixId) {
  return fetchAPI(`/api/fixes/${encodeURIComponent(fixId)}`, {
    method: 'DELETE',
  });
}
