...
class SQLiteStore:
    ...
    def load_all(self) -> Tuple[List[AnomalyEvent], List[DiagnosticReport], List[AgentTrace]]:
        with self._get_connection() as conn:
            cursor = conn.cursor()
    ...
    def save_all(self, anomalies: List[AnomalyEvent], diagnostics: List[DiagnosticReport], traces: List[AgentTrace]):
        with self._get_connection() as conn:
            cursor = conn.cursor()
