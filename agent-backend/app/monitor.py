...
class RuntimeMonitor:
    def __init__(self) -> None:
        self.config = MonitorConfig(
            ...
            gc_pause_threshold_ms=settings.gc_pause_threshold_ms,
            elu_threshold=settings.elu_threshold,
        ...
        try:
            res = self.store.load_all()
            if len(res) == 4:
                self.anomalies, self.diagnostics, self.agent_traces, self.fixes = res
            elif len(res) == 3:
    ...
    def _save(self):
        try:
            self.store.save_all(self.anomalies, self.diagnostics, self.agent_traces, self.fixes)
        except Exception as e:
    ...
    async def _detect_anomaly(self, snapshot: RawMetricSnapshot) -> AnomalyEvent | None:
        event_loop_anomaly = self._detect_event_loop_block(snapshot)
        detector_results = [
            event_loop_anomaly,
            ...
            self._detect_gc_pressure(snapshot),
            self._detect_elu_saturation(snapshot) if not event_loop_anomaly else None,
            self._detect_throughput_drop(snapshot),
        ]
    ...
    def _detect_gc_pressure(self, snapshot: RawMetricSnapshot) -> tuple[AnomalyType, float, float, str, str] | None:
        val = snapshot.gc.max_pause_ms
        active = val > self.config.gc_pause_threshold_ms
        self._set_streak(AnomalyType.gc_pressure, active)
        if not active or self._detector_streaks[AnomalyType.gc_pressure] < 2:
            return None
        sev = "critical" if val > self.config.gc_pause_threshold_ms * 3 else ("high" if val > self.config.gc_pause_threshold_ms * 1.5 else "medium")
        return (
            AnomalyType.gc_pressure,
            val,
            self.config.gc_pause_threshold_ms,
            sev,
            f"GC pause reached {val:.1f}ms (max), {snapshot.gc.gc_time_percent:.1f}% time in GC.",
        )

    def _detect_elu_saturation(self, snapshot: RawMetricSnapshot) -> tuple[AnomalyType, float, float, str, str] | None:
        val = snapshot.elu
        active = val > self.config.elu_threshold
        self._set_streak(AnomalyType.elu_saturation, active)
        if not active or self._detector_streaks[AnomalyType.elu_saturation] < 3:
            return None
        sev = "critical" if val > 0.95 else "high"
        return (
            AnomalyType.elu_saturation,
            val * 100,
            self.config.elu_threshold * 100,
            sev,
            f"Event loop utilization at {val*100:.1f}% — nearing saturation.",
        )

    def _detect_throughput_drop(self, snapshot: RawMetricSnapshot) -> tuple[AnomalyType, float, float, str, str] | None:
        if len(self.metrics_history) < 10:
            return None
        recent_rps = [s.throughput_rps for s in self.metrics_history[-5:]]
        older_rps = [s.throughput_rps for s in self.metrics_history[-15:-5]]
        if not older_rps or not recent_rps:
            return None
        avg_recent = sum(recent_rps) / len(recent_rps)
        avg_older = sum(older_rps) / len(older_rps)
        if avg_older < 1.0:
            return None
        drop_pct = ((avg_older - avg_recent) / avg_older) * 100
        active = drop_pct > self.config.throughput_drop_percent
        self._set_streak(AnomalyType.throughput_drop, active)
        if not active or self._detector_streaks[AnomalyType.throughput_drop] < 2:
            return None
        sev = "critical" if drop_pct > 80 else "high"
        return (
            AnomalyType.throughput_drop,
            drop_pct,
            self.config.throughput_drop_percent,
            sev,
            f"Throughput dropped {drop_pct:.0f}%: {avg_older:.1f} → {avg_recent:.1f} req/s.",
        )
    ...
    def metric_points(self, minutes: int | None = None) -> list[MetricPoint]:
        points = [
            MetricPoint(
                ...
                gc_pause_max_ms=s.gc.max_pause_ms,
                gc_time_percent=s.gc.gc_time_percent,
                elu=s.elu,
